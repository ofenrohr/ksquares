...
int main(int argc, char **argv)
{
    ...
    //parser.addOption(QCommandLineOption(QStringList() <<  i18n("batch-prediction"), i18n("Enable batch prediction (don't use this, it will end in deadlock)")));
    ...
    // disable evaluation in self-play
    bool noEvaluation = false;
    if (parser.isSet("no-evaluation")) {
        noEvaluation = true;
    }
    ...
    double hpTau = 1.0;
    if (parser.isSet(i18n("hp-mcts-tau"))) {
        bool ok = false;
        double tmp = parser.value(i18n("hp-mcts-tau")).toDouble(&ok);
        if (ok) {
            hpTau = tmp;
        } else {
            QMessageBox::warning(nullptr, i18n("Self-Play error"), i18n("Invalid hp-mcts-tau argument"));
        }
    }
    AlphaDots::aiAlphaZeroMCTS::tau = hpTau;

    // hp-no-move-sequences - hyperparameter for alphazero mcts
    if (parser.isSet(i18n("hp-mcts-no-move-sequences"))) {
